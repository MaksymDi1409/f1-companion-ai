...
const CURRENT_YEAR = 2026; // Якщо даних ще не буде, спробуй змінити на 2025
...
export async function getNextRaceWeekend() {
  try {
    // ЗМІНА ТУТ: тепер ми використовуємо змінну OPENF1_API
    const response = await fetch(
      `${OPENF1_API}/sessions?year=${CURRENT_YEAR}`
    );
...
export async function getDriverStandings() {
  try {
    // ЗМІНА ТУТ: тепер ми використовуємо змінну JOLPI_API
    const response = await fetch(
      `${JOLPI_API}/current/driverStandings.json`
    );
    const data = await response.json();
...
export async function getConstructorStandings() {
  try {
    // ЗМІНА ТУТ: тепер ми використовуємо змінну JOLPI_API
    const response = await fetch(
      `${JOLPI_API}/current/constructorStandings.json`
    );
    const data = await response.json();
