// lib/f1-api.js
const CURRENT_YEAR = 2026; // Якщо даних на 2026 ще нема, зміни на 2025

// ============================================
// OpenF1 API — Розклад сесій (Через твій проксі)
// ============================================

export async function getNextRaceWeekend() {
  try {
    // СЕКРЕТ ТУТ: Кодуємо посилання, щоб зберегти ?year=2026
    const targetEndpoint = encodeURIComponent(`/sessions?year=${CURRENT_YEAR}`);
    const response = await fetch(`/api/openf1?endpoint=${targetEndpoint}`);
...
// ============================================
// Jolpi API — Standings (Через твій проксі)
// ============================================

export async function getDriverStandings() {
  try {
    const targetEndpoint = encodeURIComponent('/current/driverStandings.json');
    const response = await fetch(`/api/jolpi?endpoint=${targetEndpoint}`);
    const data = await response.json();
...
export async function getConstructorStandings() {
  try {
    const targetEndpoint = encodeURIComponent('/current/constructorStandings.json');
    const response = await fetch(`/api/jolpi?endpoint=${targetEndpoint}`);
    const data = await response.json();
...
export function formatDateTime(dateString, timezone = 'Europe/Kiev') {
  try {
    ...
    return {
      date: date.toLocaleDateString('uk-UA', { weekday: 'short', month: 'short', day: 'numeric' }),
      time: date.toLocaleTimeString('uk-UA', { hour: '2-digit', minute: '2-digit', timeZone: timezone }),
      full: date.toLocaleDateString('uk-UA', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }),
    };
  } catch (error) {
...
export function translateSessionName(sessionName) {
  const translations = {
    ...
    'Qualifying': 'Кваліфікація', 'Sprint': 'Спринт', 'Sprint Qualifying': 'Спринт Кваліфікація',
    'Race': 'Гонка',
  };
